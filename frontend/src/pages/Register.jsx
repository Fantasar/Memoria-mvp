...
function Register() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!userRole && response.user?.role_id) {
        const roleMapping = { 1: 'client', 2: 'prestataire', 3: 'admin' };
  ...
  const { formData, errors, isSubmitting, handleChange, handleSubmit } = useForm(initialValues, validate, onSubmit);
  const showPrestataireFields = formData.role === 'prestataire';

  return (
    <>
      {/* Navbar identique à la page Home */}
      <Navbar />

      {/* Padding pour éviter que le formulaire soit caché par la Navbar */}
      <div className="pt-24">
        <AuthLayout title="Inscription" subtitle="Créez votre compte Mémoria">
          <form onSubmit={handleSubmit}>
            ...
            {showPrestataireFields && (
              <>
                <InputField label="Numéro SIRET" type="text" name="siret" value={formData.siret} onChange={handleChange} error={errors.siret} placeholder="12345678901234" maxLength={14} required />
                <SelectField
                ...
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-sm text-blue-700">ℹ️ Votre compte sera examiné par un administrateur avant activation.</p>
                </div>
              </>
            )}
