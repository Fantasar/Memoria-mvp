...
function CurrentMission() {
  ...
  const fetchCurrentMission = async () => {
    ...
    try {
      ...
      // Trouver la mission en cours (accepted, awaiting_validation, ou disputed)
      const inProgressOrder = orders.find(order => 
        ['accepted', 'awaiting_validation', 'disputed'].includes(order.status)
      );
      ...
      if (inProgressOrder) {
        ...
        if (['awaiting_validation', 'disputed'].includes(inProgressOrder.status)) {
          const photosResponse = await axios.get(`/api/photos/order/${inProgressOrder.id}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
      ...
      console.error('Erreur mission en cours:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      {/* Statut accepted - En attente intervention */}
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-blue-800">
                Le prestataire effectuera l'intervention à la date prévue. Vous recevrez les photos avant/après une fois le travail terminé.
              </p>
            </div>
          </div>
        </div>
